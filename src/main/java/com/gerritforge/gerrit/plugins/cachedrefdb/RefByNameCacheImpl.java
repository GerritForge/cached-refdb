...
class RefByNameCacheImpl implements RefByNameCache {
  ...
  public List<Ref> allByPrefix(String projectName, String prefix) {
    ...
    try {
      ...
      logger.atWarning().withCause(e).log("Failed to load all refs by prefix for project %s", projectName);
    }
    return refsList;
  }
  ...
  private Optional<Ref> getMaybeRef(String projectName, String refString) {
    try {
      ...
      logger.atWarning().withCause(e).log("Failed to lookup ref %s in project %s", refString, projectName);
    }
