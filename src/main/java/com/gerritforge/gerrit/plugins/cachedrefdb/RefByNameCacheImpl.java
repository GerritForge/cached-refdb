...
class RefByNameCacheImpl implements RefByNameCache {
  ...
  static com.google.inject.Module module() {
    return new CacheModule() {
      ...
      protected void configure() {
        ...
        cache(
            REFS_NAMES_BY_PREFIX,
            RefsByPrefixKey.class,
  ...
  public void evict(String identifier, String ref) {
    ...
    RefsByPrefixKey refsByPrefixKey = new RefsByPrefixKey(Project.nameKey(identifier));
    TernarySearchTree<ObjectId> tree = refsNamesByPrefix.getIfPresent(refsByPrefixKey);
    if (tree != null) {
      tree.delete(ref);
      refsNamesByPrefix.put(refsByPrefixKey, tree);
    }
  }
    ...
    try {
      TernarySearchTree<ObjectId> tree =
          refsNamesByPrefix.get(
              new RefsByPrefixKey(Project.nameKey(projectName)),
