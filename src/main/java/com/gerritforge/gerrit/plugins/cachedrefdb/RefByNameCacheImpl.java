...
class RefByNameCacheImpl implements RefByNameCache {
  ...
  public List<Ref> allByPrefix(String projectName, String prefix) throws IOException {
    try {
      ...
      List<Ref> refsList =
          refNames.stream()
              .map(refName -> getMaybeRef(projectName, refPrefix + refName))
              .filter(Optional::isPresent)
              .map(Optional::get)
              .collect(Collectors.toList());
      if (refsList.isEmpty()) {
        getMaybeRef(projectName, prefix).ifPresent(refsList::add);
        getMaybeRef(projectName, refPrefix).ifPresent(refsList::add);
      }
      return refsList;
    } catch (ExecutionException e) {
  ...
  private Optional<Ref> getMaybeRef(String projectName, String refPrefix) {
