...
class RefByNameCacheImpl implements RefByNameCache {
  ...
  private void updateRefsPrefixesCache(String projectName, String refName)
    ...
    forEeachRefsListByPrefixesCache(projectName, refName, (refs) -> refs.add(refName));
  }

  private void deleteRefFromRefsPrefixesCache(String projectName, String refName)
      throws ExecutionException {
    forEeachRefsListByPrefixesCache(projectName, refName, (refs) -> refs.remove(refName));
  }

  private void forEeachRefsListByPrefixesCache(
      String projectName, String refName, Consumer<Set<String>> refsByPrefixFunc)
      throws ExecutionException {
    ...
    for (String refPrefix : Splitter.on('/').split(refPrefixSubstring)) {
      ...
      Set<String> existingRefNames =
          refsNamesByPrefix.get(
              ...
              ConcurrentHashMap::newKeySet);
      refsByPrefixFunc.accept(existingRefNames);
    }
  }
