...
class RefByNameCacheImpl implements RefByNameCache {
  ...
  public static class RefsByProjectLoader
    ...
    public AtomicReference<TernarySearchTree<ObjectId>> load(RefsByProjectKey key) throws Exception {
  ...
  private final LoadingCache<RefsByProjectKey, AtomicReference<TernarySearchTree<ObjectId>>> refsNamesByProject;

  @Inject
  RefByNameCacheImpl(
      @Named(REF_BY_NAME) LoadingCache<String, Optional<Ref>> refByName,
      @Named(REFS_NAMES_BY_PROJECT)
          LoadingCache<RefsByProjectKey, AtomicReference<TernarySearchTree<ObjectId>>> refsNamesByProject) {
    this.refByName = refByName;
  ...
  public void evict(String identifier, String ref) {
    ...
    AtomicReference<TernarySearchTree<ObjectId>> treeRef = refsNamesByProject.getIfPresent(refsByProjectKey);
    if (treeRef != null) {
      treeRef.getAndUpdate(oldTree -> {
        oldTree.delete(ref);
        return oldTree;
      });
    }
  ...
  private void updateRefsPrefixesCache(String projectName, String refName, ObjectId oid)
    ...
    AtomicReference<TernarySearchTree<ObjectId>> targetTreeRef = treeRef != null ? treeRef : new AtomicReference<>(new TernarySearchTree<>());

    targetTreeRef.getAndUpdate(oldTree -> {
