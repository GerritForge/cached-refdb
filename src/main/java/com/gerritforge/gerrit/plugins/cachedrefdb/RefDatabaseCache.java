...
interface RefDatabaseCache {
  ...
  Set<Ref> getRefsByObjectId(CachedRefRepository repo, ObjectId id, RefDatabase delegate) throws ExecutionException;

  boolean hasFastTipsWithSha1(RefDatabase delegate);

  void removeRefFromObjectIdCache(String identifier, Ref refName, ObjectId oldId);

  void addRefToObjectIdCache(String identifier, Ref refName, ObjectId newId) throws ExecutionException;
}
