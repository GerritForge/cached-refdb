 ...
 *
 * <p>TernarySearchTree is a type of trie (sometimes called a prefix tree) where nodes are arranged
 * in a manner similar to a binary search tree, but with up to three children rather than the binary
 * tree's limit of two. Like other prefix trees, a ternary search tree can be used as an associative
 * map structure with the ability for incremental string search. However, ternary search trees are
 * more space efficient compared to standard prefix trees, at the cost of speed.
 *
 * <p>Keys must not be null or empty. Values cannot be null.
 *
...
public class TernarySearchTree<Value> {
  ...
  /**
   * Insert map of key-value pairs. Values of existing keys are overwritten. Use this method to
  ...
  /**
   * Delete a key-value pair and return the old value. Does nothing if the key doesn't exist.
   *
   * @param key the key
   * @return the value that was associated with the key, or {@code null} if the key did not exist
   */
  @Nullable
   ...
   * @return the key which is the longest prefix of the given query string or {@code null} if none
   *     exists.
   */
  @Nullable
  ...
  private void findKeysWithPrefix(Node<Value> node, StringBuilder prefix, Queue<String> keys) {
    if (node == null) {
      return;
    }
  ...
  private void findValuesWithPrefix(Node<Value> node, StringBuilder prefix, List<Value> values) {
    if (node == null) {
      return;
    }
  ...
  private void findKeysWithPrefix(
      Node<Value> node, StringBuilder prefix, int i, String pattern, Queue<String> keys) {
    ...
    if (c == WILDCARD || node.c == c) {
      ...
      if (i < pattern.length() - 1) {
        findKeysWithPrefix(node.eq, prefix.append(node.c), i + 1, pattern, keys);
        prefix.deleteCharAt(prefix.length() - 1);
      }
    }
    if (c == WILDCARD || node.c < c) {
